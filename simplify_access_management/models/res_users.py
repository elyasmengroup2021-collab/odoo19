...
class res_users(models.Model):
    ...
    def _login(self, credential, user_agent_env):
        auth_info = super(res_users, self)._login(credential, user_agent_env=user_agent_env)
        try:
            if auth_info and auth_info.get('uid'):
                if self.env['access.management'].sudo().search(
                        [('user_ids', 'in', auth_info['uid']), ('disable_login', '=', True)], limit=1):
                    raise AccessDenied()
        except AccessDenied:
            _logger.info("Login failed for login:%s", credential.get('login'))
            raise
        return auth_info
