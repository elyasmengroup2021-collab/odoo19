...
class AttendanceReportWizard(models.TransientModel):
    ...
    def _attendance_window(self, employee, operational_date, shift):
        if not shift or shift.shift_type == 'day_off':
            start = datetime.combine(operational_date, datetime.min.time())
            end = datetime.combine(operational_date, datetime.max.time())
        else:
            start, end = shift.get_local_window(operational_date)
        return start, end

    def _get_attendance_for_day(self, employee, operational_date, shift):
        start, end = self._attendance_window(employee, operational_date, shift)
        # Stored datetimes are queried with a broad UTC-safe range, then matched by
        # the computed operational_date, which is the authoritative assignment.
    ...
    def _get_leave_for_day(self, employee, operational_date):
        start = datetime.combine(operational_date, datetime.min.time())
        end = datetime.combine(operational_date, datetime.max.time())
    ...
    def get_movement_data(self):
        ...
        for emp in self._get_employees():
            ...
            while cur <= self.date_to:
                ...
                if self.shift_id and (not shift or shift.id != self.shift_id.id):
                    cur += timedelta(days=1)
                    continue
                atts = self._get_attendance_for_day(emp, cur, shift)
                leave = self._get_leave_for_day(emp, cur)
                attendance = atts[0] if atts else False
                status = 'On Leave' if leave else 'Present' if attendance else 'Absent'
                ...
                days.append({
                    ...
                    'late': attendance.late_minutes if attendance else 0,
                    'early': attendance.early_out_minutes if attendance else 0,
                    'overtime': attendance.overtime_minutes if attendance else 0,
                    'hours': round(attendance.work_duration_hours, 2) if attendance else 0,
                })
                cur += timedelta(days=1)
    ...
    def get_delay_data(self):
        ...
        for emp in self._get_employees():
            ...
            while cur <= self.date_to:
                shift = self._get_shift_on_date(emp.id, cur)
                if not self.shift_id or (shift and shift.id == self.shift_id.id):
                    for att in self._get_attendance_for_day(emp, cur, shift):
                        if att.late_minutes > 0:
                            lines.append({'code': emp.barcode or emp.id, 'name': emp.name, 'date': cur.strftime('%Y-%m-%d'), 'delay': att.late_minutes, 'shift': shift.name if shift else '', 'dept': emp.department_id.name if emp.department_id else ''})
                cur += timedelta(days=1)
        return lines
        ...
        for emp in self._get_employees():
            ...
            while cur <= self.date_to:
                ...
                if not self.shift_id or (shift and shift.id == self.shift_id.id):
                    for att in self._get_attendance_for_day(emp, cur, shift):
                        if att.check_in:
                            lines.append({'date': cur.strftime('%Y-%m-%d'), 'time': att.check_in.strftime('%H:%M'), 'type': 'IN', 'name': emp.name, 'code': emp.barcode or emp.id, 'shift': shift.name if shift else '', 'dept': emp.department_id.name if emp.department_id else ''})
                        if att.check_out:
                            lines.append({'date': cur.strftime('%Y-%m-%d'), 'time': att.check_out.strftime('%H:%M'), 'type': 'OUT', 'name': emp.name, 'code': emp.barcode or emp.id, 'shift': shift.name if shift else '', 'dept': emp.department_id.name if emp.department_id else ''})
                cur += timedelta(days=1)
        return lines
                ...
                if not self.shift_id or (shift and shift.id == self.shift_id.id):
                    for att in self._get_attendance_for_day(emp, cur, shift):
                        if not att.check_out:
                            lines.append({'date': cur.strftime('%Y-%m-%d'), 'name': emp.name, 'code': emp.barcode or emp.id, 'time': att.check_in.strftime('%H:%M') if att.check_in else '', 'shift': shift.name if shift else '', 'dept': emp.department_id.name if emp.department_id else ''})
                cur += timedelta(days=1)
        return lines
